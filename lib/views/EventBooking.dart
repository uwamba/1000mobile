...
class EventRoomForm extends StatefulWidget {
  final Map<String, dynamic> event;


  const EventRoomForm({required this.event, super.key});

  @override
  EventNowFormState createState() => EventNowFormState();
}

class EventNowFormState extends State<EventRoomForm> {
  ...
  double get totalAmount {
    final pricePerPerson = widget.event['price_per_person'];
    final packagePrice = widget.event['package_price'];

    final price = (pricePerPerson != null && pricePerPerson != 0)
        ? pricePerPerson
        : (packagePrice != null && packagePrice != 0)
        ? packagePrice
        : 0;

    return (daysStayed > 0 ? daysStayed : 0) * (price as num).toDouble();
  }
  ...
  Future<void> verifyOTPAndBook() async {
    ...
    try {
      ...
      final bookingData = {
        ...
        'address': address,
        'phone': phoneNumber,
        'journey': widget.event,
        'object_type': 'event',
        'object_id': widget.event['id'],
      ...
      body: SingleChildScrollView(
        ...
        step == "form"
          ...
          child: Column(
            ...
            children: [
              Text(
                '🛏️ Book event at ${widget.event['name'] ?? 'Hotel'}',
                style: TextStyle(
                  fontSize: 22,
